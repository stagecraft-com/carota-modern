 ...
 */
export const MULTIPLE_VALUES = Symbol('multipleValues');
...
export interface Bounds {
  ...
  w: number; // width
  h: number; // height
  r: number; // right (computed: l + w)
  b: number; // bottom (computed: t + h)
  center?(): Point;
}
...
export interface BlockCode extends InlineCode {
  ...
  block?: (
    ...
    parent: CarotaNode,
    formatting: PartialFormatting
  ) => ((word: Word) => CarotaNode | undefined) | undefined;
}
...
export interface Document {
  ...
  on<K extends keyof DocumentEvents>(
    event: K,
    handler: (...args: DocumentEvents[K]) => void
  ): void;
